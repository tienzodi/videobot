...
class YoutubeVideoController extends AppController
{
    ...
    public function cronUpdateVideos($language){
        ...
        foreach($this->CategoriesList as $category_id=>$categoryName)
        {
            ...
            $video = $this->Videos->find('all', array('order' => array('video_published_at' => 'DESC')))->where(['category_id'=>$category_id,'search_language'=>$content_language])->limit(1);
            ...
            if($response->isOk())
            {
                ...
                foreach($items as $item){
                    ...
                    $video_data[$item['id']['videoId']] = array(
                        ...
                        'status' => 0,
                        'search_type' => 2,
                        'search_language' => $content_language,
                        'activate' => 1,
                    );
                }
